    ...
    waitfor_w_r_queue:
        ...
        if (alarm_w_r_queue->is_read == true) //讀數據
        {
            ...
            if (alarm_w_r_queue->pos == 0) //文件數據流首次讀取
            {
                ...
                do
                {
                    if (strncmp(alarm_file_cache->alarm_ID, alarm_w_r_queue->alarm_ID, ALARM_ID_LEN) == 0)
                    {
                        alarm_w_r_queue->total_len = alarm_file_cache->file_len;
                        if (alarm_file_cache->file_len > SOUND_STREAM_DEFAULT_LENGTH)
                            alarm_w_r_queue->len = SOUND_STREAM_DEFAULT_LENGTH;
                        else
                            alarm_w_r_queue->len = alarm_file_cache->file_len;
                ...
                if ((uint8_t *)alarm_file_cache >= (uint8_t *)eland_sound_point + sizeof(eland_sound_point))
                {
